...
const createReclamation = async (req, res) => {
  try {
    ...
    const pdfPath = "files/reclamationPersonnelFiles/pdf/";
    const galleryPath = "files/reclamationPersonnelFiles/photo/";
    const videoPath = "files/reclamationPersonnelFiles/video/";

    const pdfFilename = globalFunctions.generateUniqueFilename(pdfExtension, 'reclamationPersonnelPDF');
    const galleryFilenames = galleryExtensions.map((ext, index) => 
      globalFunctions.generateUniqueFilename(ext, `reclamationPersonnelPHOTO_${index}`)
    );
    const videoFilename = globalFunctions.generateUniqueFilename(videoExtension, 'reclamationPersonnelVIDEO');
    ...
    const reclamationPersonnel = await reclamationService.createReclamation({
      personnelId,
      title,
      description,
      response,
    ...
    res.status(201).json(reclamationPersonnel);
  } catch (error) {
    console.error("Error creating ReclamationPersonnel:", error);
